...
export function NewProjectForm() {
  ...
  useEffect(() => {
    api.config().then((c) => setMock(c.mock)).catch(() => undefined);
  }, []);
  ...
  return (
    <div className="space-y-6">
      {mock && (
        <div className="rounded-md border border-amber-700/60 bg-amber-950/30 px-4 py-2 text-xs text-amber-200">
          MOCK MODE: no OPENAI_API_KEY configured. Transcription / analysis / ranking use local heuristics and
          placeholder images. See README for setup.
        </div>
      )}
